...
blogs_schema = { 
    ...
    { 'name': 'page_last_modified', 'type': 'int64', 'facet': True, 'index': False }, #time data is stored as: int64->unix time
...
# typesense will index the title and body, given by the json data, parsed by me earlier.


# future processing can add for documents:
() Boost factors
() Weights
() higher value for anchor elements..

# 'url' and 'preview' fields do not need to be indexed, i.e searched upon. They only need to be stored on disk with the document, to be fetched when the document is requested.

There will be no inherent rating for the article currently. Hence, if the inverted index thinks it will fit according to the keywords provided and intention, 
This is the doc that will be served.

# Should title be the default_sorting_field? maybe. And maybe it should simply be given a larger weight. Say 60%.
"""
...
try:   # Create the collection if it hasn't been created yet.
    client.collections.create(books_schema)
except Exception as e:
    print('Collection with books schema has already been created')
    #print(f'here is the collection schema: {client.collections.retrieve()}')
    pass

with open('/tmp/books.jsonl') as jsonl_file:
    print('tring to import jsonl_file again! response:')
    print(client.collections['books'].documents.import_( # needs to to upsert
...
self.domain = (self.home_page.split('://')[-1]).split('/')[0] # split http://lo.nyu.com/h/bo to only domain
