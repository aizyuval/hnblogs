...
from scrapy.linkextractors import (
    LinkExtractor,
    IGNORED_EXTENSIONS,
)   # LinkExtractor is the same as LxmlLinkExtractor
...
from indexer.spiders.hnblogs_parser import customparser
import re


IGNORED_EXTENSIONS += ['jar', 'json', 'cbr', 'xml']
...
class HNblogsSpider(CrawlSpider):
    ...
    def __init__(self, *args, **kwargs):
        # Get kwargs
        self.typesense_helper = kwargs.get(
            'typesense_helper'
        ...
        super(HNblogsSpider, self).__init__(
            self, *args, **kwargs
        )   # invoking the __init__ function of the CrawlSpider class, to not break crucial initialisations
    ...
    def start_requests(self):
        for (
            url
        ) in self.start_urls:   # there's only one, so it does not matter.
            yield Request(url)
...
"""
        ...
        if isinstance(response, HtmlResponse):
            ...
            logger.warn(
                'Skipping processing start_url: {}'.format(response.url)
            )
    ...
    def parse_item(self, response):
        ...
        ):   # Text response is a class that is initiated when the response is html/xml
            logger.info(
                'Item {} is not a TextResponse, so skipping.'.format(response)
            )   # Skip item if e.g. an image
        else:
            item = customparser(response, self.domain, self.site_config)
            if item:
